...
def check_config(config: Config) -> Config:
        ...
        if backend_config.type == "openai":
            ...
            if "api_key" not in arguments or not arguments["api_key"]:
                api_key = (
                    ...
                    if backend_config.type == "openai"
                    else get_env_var(f"{backend_config.type.upper()}_API_KEY")
                )
                if api_key:
            ...
            if "base_url" not in arguments or not arguments["base_url"]:
                if backend_config.type == "openai":
                    arguments["base_url"] = get_env_var(
    ...
    for benchmark_config in config.evaluation.benchmark:
        ...
        if (
            not hasattr(benchmark_config, "num_samples")
            or benchmark_config.num_samples is None
        ):
            benchmark_config.num_samples = get_env_var("DEFAULT_NUM_SAMPLES", "1", int)

        if (
            not hasattr(benchmark_config, "num_workers")
            or benchmark_config.num_workers is None
        ):
            benchmark_config.num_workers = get_env_var("DEFAULT_NUM_WORKERS", "1", int)

        if (
            not hasattr(benchmark_config, "pass_at_k")
            or benchmark_config.pass_at_k is None
        ):
            benchmark_config.pass_at_k = get_env_var("DEFAULT_PASS_AT_K", "1", int)

        if (
            not hasattr(benchmark_config, "log_batch_size")
            or benchmark_config.log_batch_size is None
        ):
            benchmark_config.log_batch_size = get_env_var(
                "DEFAULT_LOG_BATCH_SIZE", None, int
            )

        # Ensure generate_args exists and populate default values
        if (
            not hasattr(benchmark_config, "generate_args")
            or not benchmark_config.generate_args
        ):
        ...
        if "temperature" not in generate_args:
            generate_args["temperature"] = get_env_var(
                "DEFAULT_TEMPERATURE", "0.0", float
            )
    ...
    if (
        not hasattr(config.evaluation.output, "result_folder")
        or not config.evaluation.output.result_folder
    ):
        config.evaluation.output.result_folder = get_env_var(
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Log result by batch",
    )
    parser.add_argument(
        "--log-batch-size", type=int, default=1, help="Frequency to log result"
    )
    ...
    global_results_save_path = os.path.join(
        save_folder, get_env_var("GLOBAL_RESULTS_FILENAME", "results.json")
    )
    ...
    for bi, backend_config in enumerate(backend_configs):
        ...
        for bmi, benchmark_config in enumerate(benchmark_configs):
            ...
            num_workers = (
                benchmark_config.num_workers if benchmark_config.num_workers else 1
            )
            ...
            config_save_path = os.path.join(
                benchmark_result_folder, get_env_var("CONFIG_FILENAME", "config.json")
            )
            prompts_save_path = os.path.join(
                benchmark_result_folder,
                get_env_var("PROMPTS_FILENAME", "prompts.jsonl"),
            )
            generations_save_path = os.path.join(
                benchmark_result_folder,
                get_env_var("GENERATIONS_FILENAME", "generations.jsonl"),
            )
            solutions_save_path = os.path.join(
                benchmark_result_folder,
                get_env_var("SOLUTIONS_FILENAME", "solutions.jsonl"),
            )
            evalutions_save_path = os.path.join(
                benchmark_result_folder,
                get_env_var("EVALUATIONS_FILENAME", "evaluations.jsonl"),
            )
            result_save_path = os.path.join(
                benchmark_result_folder, get_env_var("RESULT_FILENAME", "result.jsonl")
            )

            # Save benchmark config
            with open(config_save_path, "w", encoding="utf-8") as fp:
                json.dump(
                    dict(config_dict_for_signature), fp, indent=2, ensure_ascii=False
                )
            ...
            for batch_start in tqdm(range(0, len(prompts), log_batch_size)):
                ...
                generations = backend.generate(
                    batch_prompts, generate_args=generate_args, num_samples=num_samples
                )
            ...
            result_list = group_and_count(
                evaluations, group_key="task_id", count_key="passed"
            )
            pass_rate = float(
                np.mean(
                    estimate_pass_at_k(
                        num_samples=num_samples, num_correct=result_list, k=pass_at_k
                    )
                )
            )
            ...
            global_results.update(
                {os.path.relpath(benchmark_result_folder, save_folder): result}
            )
