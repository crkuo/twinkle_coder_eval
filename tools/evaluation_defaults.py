...
def apply_evaluation_defaults(
    config: Optional[Dict[str, Any]] = None, args: Optional[Any] = None
) -> Dict[str, Any]:
    """
    應用預設值到配置和參數中
    ...
    if config:
        ...
        if "backend" in config and isinstance(config["backend"], list):
            for backend_config in config["backend"]:
                ...
                if "model_name" not in backend_config:
                    backend_config["model_name"] = backend_config.get(
                        "type", "unknown_model"
                    )
        ...
        if "evaluation" in config:
            ...
            # 處理 benchmark 配置
            if "benchmark" in eval_config and isinstance(
                eval_config["benchmark"], list
            ):
            ...
            if "result_folder" not in eval_config["output"]:
                eval_config["output"]["result_folder"] = defaults[
...
def setup_evaluation_environment(config: Dict[str, Any]) -> None:
    ...
    save_folder = (
        config.get("save_dir")
        or config.get("evaluation", {}).get("output", {}).get("result_folder")
        or defaults["default_result_folder"]
    )
...
def get_file_paths(
    benchmark_result_folder: str, file_names: Optional[Dict[str, str]] = None
) -> Dict[str, str]:
    """
    取得所有結果文件的路徑
...
def get_worker_count(
    config_workers: Optional[int] = None, task_count: Optional[int] = None
) -> int:
    """
    取得適當的 worker 數量
...
if __name__ == "__main__":
    ...
    sample_config = {
        "evaluation": {
            "benchmark": [{"type": "MBPP"}, {"type": "HumanEval", "num_samples": 10}]
        }
    }
