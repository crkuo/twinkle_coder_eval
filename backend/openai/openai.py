...
class OpenaiGenerator(Generator):
    def __init__(
        self, model_name: str, arguments: dict, model_type: str = "Chat"
    ) -> None:
        ...
        if "api_key" not in client_args or not client_args["api_key"]:
            ...
            if env_api_key:
                ...
                raise ValueError(
                    "API key not found in config or environment variables. Please set OPENAI_API_KEY in .env file"
                )
    ...
    def generate_with_stream_auto_continue(
        self, prompt, create_params, max_rounds=None
    ):
        """
        ...
        for round_idx in range(max_rounds):
            try:
                ...
                current_messages += [
                    ...
                    {
                        "role": "user",
                        "content": "Please continue where you left off, without repeating.",
                    },
                ]
    ...
    def generate_with_normal_mode(
        self, prompt: str, create_params: dict, max_rounds=20
    ):
        s = time.time()
    ...
    def generate(
        ...
        for batch_start in range(0, len(sample_prompts), batch_size):
            ...
            for prompt_data in batch:
                try:
                    ...
                    if use_stream:
                        generation_result = self.generate_with_stream_auto_continue(
                            full_prompt, create_params
                        )
                    else:
                        generation_result = self.generate_with_normal_mode(
                            full_prompt, create_params
                        )
        ...
        grouped_generations = [
            generations[i : i + num_samples]
            for i in range(0, len(generations), num_samples)
        ]
        assert len(grouped_generations) == len(prompt_set)
        assert all(
            len(grouped_generations[i]) == num_samples
