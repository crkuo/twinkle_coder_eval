    ...
    def generate(
        ...
        if self.is_chat():
            for prompt in prompt_set:
                prompt["prompt"] = make_chat_prompt(
                    prompt["prompt"], self.tokenizer, response_prefix
                )
        ...
        for batch_start in tqdm(range(0, len(prompt_set), self.batch_size)):
            ...
            for prompt, output in zip(batch_prompt, batch_outputs):
                ...
                if not self.is_chat():
                    ...
                    completions = [
                        refine_text(response_prefix + sample + response_suffix)
                        for sample in completions
                    ]
        ...
        assert (
            len(generation_set) == len(prompt_set) * num_samples
        ), "Number of generations does not match the expected number."
